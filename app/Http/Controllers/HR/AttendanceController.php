    ...
    public function store(Request $request)
{
    ...
    $partTimeType = $employee->part_time_type ?? null;
    ...
    $overtimeMinutes = 0;
    if ($isPartTimeHours) {
        $dailyRequiredMinutes = ($employee->monthly_hours_required / 30) * 60; 
        if ($workedMinutes > $dailyRequiredMinutes) {
            $overtimeMinutes = (int)($workedMinutes - $dailyRequiredMinutes);
        }
    } else {
        if ($requiredHours > 0 && $workedMinutes > ($requiredHours * 60)) {
    ...
    if ($isPartTimeHours) {
        ...
        $requiredMonthlyHours = $employee->monthly_hours_required;
   ...
   public function update(Request $request, $employeeId)
{
    ...
    if ($isPartTimeHours) {
        ...
        $requiredMonthlyHours = (float) ($employee->monthly_hours_required ?? 0);
...
public function destroy(Attendance $attendance)
{
    DB::transaction(function () use ($attendance) {
        optional($attendance->day)->delete();
        $attendance->delete();
    });
